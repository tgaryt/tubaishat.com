...
<section id="experience" aria-labelledby="experience-heading" class="py-16 sm:py-20 relative">
	...
	<div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
		<header class="text-center mb-12 sm:mb-16">
			<h2 id="experience-heading" class="text-3xl sm:text-4xl md:text-5xl font-bold inline-block"><?= htmlspecialchars($title_parts[0] ?? '', ENT_QUOTES, 'UTF-8') ?><span class="text-secondary"><?= htmlspecialchars($experience['title_highlight'], ENT_QUOTES, 'UTF-8') ?></span><?= htmlspecialchars($title_parts[1] ?? '', ENT_QUOTES, 'UTF-8') ?></h2>
						...
						<header class="mb-4 pb-4 border-b border-secondary/20">
							<div class="flex flex-col xl:flex-row xl:items-start xl:justify-between gap-3">
								<div class="flex-1">
									...
									<p class="text-lg lg:text-xl font-semibold text-light"><?= htmlspecialchars($job['title'], ENT_QUOTES, 'UTF-8') ?></p>
									<p class="mt-1 text-sm text-gray-400"><?= htmlspecialchars($job['employment_type'], ENT_QUOTES, 'UTF-8') ?></p>
								</div>
								<div class="flex items-center gap-2 self-start bg-dark/50 border border-secondary/20 px-3 py-1.5 rounded-lg text-sm text-gray-400">
									...
									<span>
										<time datetime="<?= htmlspecialchars($job['datetime_start'], ENT_QUOTES, 'UTF-8') ?>"><?= htmlspecialchars($job['period_start'], ENT_QUOTES, 'UTF-8') ?></time>
										-
										<?php if ($job['datetime_end']): ?>
											<time datetime="<?= htmlspecialchars($job['datetime_end'], ENT_QUOTES, 'UTF-8') ?>"><?= htmlspecialchars($job['period_end'], ENT_QUOTES, 'UTF-8') ?></time>
										<?php else: ?>
											Present
										<?php endif; ?>
									</span>
						...
						<ul class="space-y-3 text-gray-300">
							<?php foreach ($job['responsibilities'] as $responsibility): ?>
								<li class="flex items-start gap-3">
									...
									<span class="text-sm lg:text-base"><?= htmlspecialchars($responsibility, ENT_QUOTES, 'UTF-8') ?></span>
								</li>
							<?php endforeach; ?>
						</ul>

						<footer class="mt-6 pt-6 border-t border-secondary/10">
							<h4 class="mb-3 text-sm font-medium text-gray-400"><?= htmlspecialchars($experience['technologies_label'], ENT_QUOTES, 'UTF-8') ?></h4>
