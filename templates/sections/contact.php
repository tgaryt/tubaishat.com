...
<section id="contact" aria-labelledby="contact-heading" class="py-16 sm:py-20 bg-linear-to-b from-primary/50 to-dark/50 relative">
	...
	<div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
		<header class="text-center mb-12 sm:mb-16">
			<h2 id="contact-heading" class="text-3xl sm:text-4xl md:text-5xl font-bold inline-block"><?= htmlspecialchars($title_parts[0] ?? '', ENT_QUOTES, 'UTF-8') ?><span class="text-secondary"><?= htmlspecialchars($contact['title_highlight'], ENT_QUOTES, 'UTF-8') ?></span><?= htmlspecialchars($title_parts[1] ?? '', ENT_QUOTES, 'UTF-8') ?></h2>
			...
			<p class="mx-auto mt-6 max-w-2xl text-base sm:text-lg text-gray-300">
				<?= htmlspecialchars($contact['intro'], ENT_QUOTES, 'UTF-8') ?>
			</p>
		</header>
		...
		<div class="grid grid-cols-1 lg:grid-cols-[1fr_1.2fr] gap-6 lg:gap-8">
			<article aria-labelledby="contact-channels-heading" class="bg-dark/40 backdrop-blur-xs border border-secondary/20 rounded-xl p-6 sm:p-8 shadow-xl">
				<h3 id="contact-channels-heading" class="text-xl sm:text-2xl font-bold text-secondary mb-6"><?= htmlspecialchars($contact['channels_heading'], ENT_QUOTES, 'UTF-8') ?></h3>
							...
							<a href="<?= htmlspecialchars($href, ENT_QUOTES, 'UTF-8') ?>"
							   ...
							   <?php if ($is_external): ?>target="_blank" rel="noopener noreferrer me"<?php endif; ?>>
								...
								<div class="min-w-0 flex-1">
									<div class="text-xs text-gray-400"><?= htmlspecialchars($item['type'], ENT_QUOTES, 'UTF-8') ?></div>
									<div class="text-sm text-light truncate"><?= htmlspecialchars($display, ENT_QUOTES, 'UTF-8') ?></div>
								</div>
							</a>
						<?php else: ?>
							<div class="flex items-start gap-4 rounded-lg border border-secondary/10 bg-dark/50 p-4">
								...
								<div class="min-w-0 flex-1">
									<div class="text-xs text-gray-400"><?= htmlspecialchars($item['type'], ENT_QUOTES, 'UTF-8') ?></div>
									<div class="text-sm text-light"><?= htmlspecialchars($display, ENT_QUOTES, 'UTF-8') ?></div>
								</div>
							</div>
						<?php endif; ?>
					<?php endforeach; ?>
			...
			<article aria-labelledby="contact-form-heading" class="bg-dark/40 backdrop-blur-xs border border-secondary/20 rounded-xl p-6 sm:p-8 shadow-xl">
				<h3 id="contact-form-heading" class="text-xl sm:text-2xl font-bold text-secondary mb-2"><?= htmlspecialchars($contact['form_heading'], ENT_QUOTES, 'UTF-8') ?></h3>
				<p class="text-sm text-gray-400 mb-6"><?= htmlspecialchars($contact['form_subtitle'], ENT_QUOTES, 'UTF-8') ?></p>

				<form id="contactForm" action="/contact" method="post" novalidate autocomplete="on" class="space-y-5">
					<input type="hidden" name="_csrf" value="<?= htmlspecialchars($csrf_token, ENT_QUOTES, 'UTF-8') ?>">

					<div class="absolute w-px h-px overflow-hidden" aria-hidden="true">
					...
					<div class="grid grid-cols-1 sm:grid-cols-2 gap-5">
						<div>
							<label for="contact-name" class="block text-sm font-medium text-light mb-1.5">
								<?= htmlspecialchars($fields['name']['label'], ENT_QUOTES, 'UTF-8') ?> <span class="text-secondary" aria-hidden="true">*</span>
							</label>
							<input type="text"
							       id="contact-name"
							       name="name"
							       ...
							       autocomplete="name"
							       placeholder="<?= htmlspecialchars($fields['name']['placeholder'], ENT_QUOTES, 'UTF-8') ?>"
							...
							<p id="contact-name-error" data-error="name" class="mt-1 text-xs text-red-400" role="status"></p>
						</div>

						<div>
							<label for="contact-company" class="block text-sm font-medium text-light mb-1.5">
								<?= htmlspecialchars($fields['company']['label'], ENT_QUOTES, 'UTF-8') ?>
							</label>
							<input type="text"
							       id="contact-company"
							       name="company"
							       ...
							       placeholder="<?= htmlspecialchars($fields['company']['placeholder'], ENT_QUOTES, 'UTF-8') ?>"
							       aria-describedby="contact-company-error"
							       class="w-full rounded-lg border border-secondary/20 bg-dark/50 px-3 py-2.5 text-light placeholder:text-gray-500 focus:outline-hidden focus-visible:ring-2 focus-visible:ring-secondary/50 focus:border-secondary aria-[invalid=true]:border-red-400">
							<p id="contact-company-error" data-error="company" class="mt-1 text-xs text-red-400" role="status"></p>
						</div>
					</div>

					<div>
						<label for="contact-email" class="block text-sm font-medium text-light mb-1.5">
							<?= htmlspecialchars($fields['email']['label'], ENT_QUOTES, 'UTF-8') ?> <span class="text-secondary" aria-hidden="true">*</span>
						</label>
						<input type="email"
						       id="contact-email"
						       name="email"
						       ...
						       autocomplete="email"
						       inputmode="email"
						       placeholder="<?= htmlspecialchars($fields['email']['placeholder'], ENT_QUOTES, 'UTF-8') ?>"
						       aria-describedby="contact-email-error"
						       class="w-full rounded-lg border border-secondary/20 bg-dark/50 px-3 py-2.5 text-light placeholder:text-gray-500 focus:outline-hidden focus-visible:ring-2 focus-visible:ring-secondary/50 focus:border-secondary aria-[invalid=true]:border-red-400">
						<p id="contact-email-error" data-error="email" class="mt-1 text-xs text-red-400" role="status"></p>
					</div>
					...
					<div>
						...
						<select id="contact-inquiry-type"
						        name="inquiry_type"
						        required
						        aria-describedby="contact-inquiry_type-error"
						...
						<p id="contact-inquiry_type-error" data-error="inquiry_type" class="mt-1 text-xs text-red-400" role="status"></p>
					</div>

					<div>
						<label for="contact-subject" class="block text-sm font-medium text-light mb-1.5">
							<?= htmlspecialchars($fields['subject']['label'], ENT_QUOTES, 'UTF-8') ?> <span class="text-secondary" aria-hidden="true">*</span>
						</label>
						<input type="text"
						       id="contact-subject"
						       name="subject"
						       ...
						       placeholder="<?= htmlspecialchars($fields['subject']['placeholder'], ENT_QUOTES, 'UTF-8') ?>"
						       aria-describedby="contact-subject-error"
						       class="w-full rounded-lg border border-secondary/20 bg-dark/50 px-3 py-2.5 text-light placeholder:text-gray-500 focus:outline-hidden focus-visible:ring-2 focus-visible:ring-secondary/50 focus:border-secondary aria-[invalid=true]:border-red-400">
						<p id="contact-subject-error" data-error="subject" class="mt-1 text-xs text-red-400" role="status"></p>
					</div>

					<div>
						<label for="contact-message" class="block text-sm font-medium text-light mb-1.5">
							<?= htmlspecialchars($fields['message']['label'], ENT_QUOTES, 'UTF-8') ?> <span class="text-secondary" aria-hidden="true">*</span>
						</label>
						<textarea id="contact-message"
						          ...
						          rows="6"
						          placeholder="<?= htmlspecialchars($fields['message']['placeholder'], ENT_QUOTES, 'UTF-8') ?>"
						...
						<p id="contact-message-error" data-error="message" class="mt-1 text-xs text-red-400" role="status"></p>
					</div>
					...
					<div class="flex flex-col sm:flex-row gap-3 sm:items-center sm:justify-between pt-2">
						<button type="submit"
							...
							<span><?= htmlspecialchars($form['submit_label'], ENT_QUOTES, 'UTF-8') ?></span>
						</button>
